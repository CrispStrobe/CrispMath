...
1. **Stop shipping fake simplify/factor** (high / L) — implement real `simplify`+`factor` via SymEngine C++ in the wrapper, or honestly degrade the UI. Most damaging correctness gap. *(repos: all 3)*
   - **`factor` FIXED in Dart 2026-05-31** (`SymbolicWeb.factor`): real univariate-over-ℚ factoring (leading coeff + rational-root linear factors w/ multiplicity + exact division; irreducible remainders reported intact). `CalculatorEngine.factor` tries it first on **every** platform, falling through to the bridge only for non-polynomial input. Verified upstream first — the C-wrapper alias is still live on bridge `main`/math-stack `master` and *can't* be fixed in C (SymEngine's C API has no symbolic factorization; boostmp WASM has no FLINT; `emcc` not installed to rebuild). `simplify` gets a Dart expand-fallback on web (mirrors the wrapper alias). Multivariate/transcendental `factor`/`simplify` remain bridge-aliased (deferred to a real C++/FLINT wrapper).
